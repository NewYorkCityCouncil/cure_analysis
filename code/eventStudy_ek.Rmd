...
#get change in funding
nypdFunds <- nypdFunds %>% group_by(precinct) %>% mutate(fundsChange = c(NA,diff(nypdFunds)))
nypdFunds <- nypdFunds %>% group_by(precinct) %>% mutate(lagFunds = lag(nypdFunds))
...
#get change in headcount
nypdHC <- nypdHC %>% group_by(precinct) %>% mutate(HCChange = c(NA,diff(nypdHC)))
nypdHC <- nypdHC %>% group_by(precinct) %>% mutate(lagHC = lag(nypdHC))
...
temp2 <- temp %>% filter(year>=2012)
summary(temp2$numShootings)
...
temp2 <- temp %>% filter(curePrecinct & year>=2012)
summary(temp2$numShootings)
