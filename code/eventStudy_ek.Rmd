...
#first/earliest row---this keeps the (earliest) fully operational Cure program (and
#the start up program for precinct 67)
...
#Use 2019 and earlier fully operational programs as our Cure precincts,
#want to study dynamic effects
program_dates <- program_dates %>% 
  mutate(curePre2020 = (yearCure < 2020))
program_dates %>% count(curePre2020)

#Flag early, mid, late adopters to explore spillover effects
program_dates <- program_dates %>% 
  mutate(earlyCure = (yearCure < 2015),
         midCure = (yearCure %in% c(2015, 2016)),
         lateCure = (yearCure == 2019))

programMerge <- program_dates %>% 
  select(precinct, yearCure, monthCure, wavesCure, curePre2020, earlyCure, midCure, lateCure)
# programMerge <- programMerge %>% 
#   mutate(yearCure = ifelse(!curePre2020,NA,yearCure),
#          monthCure = ifelse(!curePre2020,NA,monthCure),
#          wavesCure=ifelse(!curePre2020,NA,wavesCure))
...
#indicator to distinguish cure precincts from non-cure (incl. those w/ curePre2020==False) 
shootings_cure <- shootings_cure %>% 
  mutate(curePrecinct = (curePre2020==TRUE & !is.na(curePre2020)))
#indicator that Cure is currently "on"
shootings_cure <- shootings_cure %>% 
  mutate(cureOn = (curePrecinct==TRUE & year>=yearCure))

#To explore spillovers, set treatment date to earliest treatment date for the 
# "nearby" precincts (can play around with how this is defined)
shootings_cure <- shootings_cure %>%
  mutate(yearCureSO = ifelse(boro %in% c('BROOKLYN','QUEENS'), 2012, yearCure)) %>% 
  mutate(yearCureSO = ifelse(boro %in% c('MANHATTAN','BRONX') | precinct==114, 2013, yearCureSO)) %>% 
  mutate(yearCureSO = ifelse(is.na(curePre2020), NA, yearCureSO))

#Create indicators for event study
shootings_cure <- shootings_cure %>% 
  mutate(year_offset = year-yearCure,
         year_offsetSO = year-yearCureSO)
    ...
    if (i > lb & i < ub) {
      temp <- temp %>% 
        mutate(!!sym(newVar) := (!is.na(year_offset) & year_offset==i),
               !!sym(newVarSO) := (!is.na(year_offsetSO) & year_offsetSO==i))
    } else if (i==lb) {
      temp <- temp %>% 
        mutate(!!sym(newVar) := (!is.na(year_offset) & year_offset<=lb),
               !!sym(newVarSO) := (!is.na(year_offsetSO) & year_offsetSO<=lb))
        
    } else {
      temp <- temp %>% 
        mutate(!!sym(newVar) := (!is.na(year_offset) & year_offset>=ub),
               !!sym(newVarSO) := (!is.na(year_offsetSO) & year_offsetSO>=ub))
    }
  }
  return(temp)
}
...
########
df_analysis <- shootings_cure %>% filter(is.na(curePre2020) | curePre2020)
...
temp <- shootings_cure %>% filter(!is.na(curePre2020)) %>% group_by(curePre2020, year) %>% summarise(numShootings = mean(numShootings))
...
#make predictions
predictions <- predict(model2, interval='confidence', level=0.95)
temp <- cbind(df_analysis, predictions)
temp <- temp %>% filter(curePrecinct==TRUE) %>% group_by(year) %>% 
...
ggplot(data=temp) +
  ...
  #ylim(15,70)+
  labs(x="Year", y="Number of Shootings (per precinct)",
       title="Avg. Number of Shootings, Cure vs. Counterfactual",
       color="", caption="95% CIs are shown around the counterfactual") +
