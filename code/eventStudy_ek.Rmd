...
#indicator to distinguish cure precincts from non-cure (incl. those who get cure post 2019) 
...
model2 <- lm(numShootings ~ factor(precinct) + factor(year) + lag6 + lag5 + lag4 + 
              lag3 + lag2 + lag0 + lead1 + lead2 + lead3, data=shootings_cure)
#zero out event-time dummies
model2$coefficients[93:101] = 0
#make predictions
predictions <- predict(model, newdata=shootings_cure, interval='confidence', level=0.95)
